...
describe('WaitingRoomListComponent', () => {
    ...
    beforeEach(() => {
        ...
        socketServiceMock.getUser.and.returnValue(of({ username: 'currentUser' } as User));

        waitingRoomServiceMock = jasmine.createSpyObj('WaitingRoomService', ['isBannable', 'filterBannedUsers']);
    ...
    it('should update userList when a new user joins', () => {
        const newUser: User = { username: 'newUser', answered: false };
    ...
    it('should get the current user on ngOnInit', () => {
        const currentUserMock: User = { username: 'CurrentUser', answered: false };
    ...
    it('should remove a user and navigate when onUserLeft is called and the user is bannable', () => {
        const bannedUser: User = { username: 'BannedUser', answered: false };
        component['userList'] = [bannedUser, { username: 'OtherUser', answered: false }];
        component['user'] = { username: 'CurrentUser', answered: false };
        waitingRoomServiceMock.isBannable.and.returnValue(true);
        waitingRoomServiceMock.filterBannedUsers.and.returnValue([]);
    ...
    it('should not navigate when onUserLeft is called and the user is not bannable', () => {
        const nonBannedUser: User = { username: 'NonBannedUser', answered: false };
        component['userList'] = [nonBannedUser, { username: 'OtherUser', answered: false }];
        component['user'] = { username: 'CurrentUser', answered: false };
        waitingRoomServiceMock.isBannable.and.returnValue(false);
        waitingRoomServiceMock.filterBannedUsers.and.returnValue([nonBannedUser]);

        component.ngOnInit();
        socketServiceMock.onUserLeft.calls.mostRecent().args[0](nonBannedUser.username);

        expect(component['userList'].find((u) => u.username === nonBannedUser.username)).toBe(nonBannedUser);
